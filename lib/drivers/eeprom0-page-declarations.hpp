...
namespace CSdrivers {
    ...
    //----------PAGE_000---------------
    constexpr auto COMPANY_NAME_PAGE = PageId::PAGE_000;

    struct PageCompany {
        char companyName [sizeof(COMPANY_NAME)] = COMPANY_NAME;  // Includes null byte
        uint8_t unused [EEP_PAGE_SIZE >= sizeof(companyName)
                                ? EEP_PAGE_SIZE - sizeof(companyName)
                                : 0]{};  // Expansion area
    };
    static_assert(sizeof(PageCompany) <= EEP_PAGE_SIZE, "EeProm0PageCompany declaration too big!!");
    ...
    //----------PAGE_001---------------
    constexpr auto PRODUCT_INFO_1 = PageId::PAGE_001; // "Focus Rack 500" (14+1)

    struct PageProductInfo1 {
        char productName [sizeof(PRODUCT_NAME)] = PRODUCT_NAME;  // Includes null byte
        uint8_t unused [EEP_PAGE_SIZE >= sizeof(productName)
                        ? EEP_PAGE_SIZE - sizeof(productName)
                        : 0]{};  // Expansion area
    };
    static_assert(sizeof(PageProductInfo1) <= EEP_PAGE_SIZE, "EeProm0PageProductInfo1 declaration too big!!");
    ...
    //----------PAGE_002---------------
    constexpr auto PRODUCT_INFO_2 = PageId::PAGE_002; // part # (10+1); serial # (16+1)
    ...
    struct PageProductInfo2 {
        ...
        uint8_t unused [EEP_PAGE_SIZE >= (sizeof(partNumber) + sizeof(serialNumber))
                        ? EEP_PAGE_SIZE - (sizeof(partNumber) + sizeof(serialNumber))
                        : 0]{};  // Expansion area
    };
    static_assert(sizeof(PageProductInfo2) <= EEP_PAGE_SIZE, "EeProm0PageProductInfo2 declaration too big!!");
    ...
    //----------PAGE_003---------------
    constexpr auto PRODUCT_INFO_3 = PageId::PAGE_003; // firmware version (3 bytes)

    struct PageProductInfo3 {
        uint8_t majorVersion = 0;
        uint8_t minorVersion = 0;
        uint16_t buildNumber = 0;
        uint8_t unused [EEP_PAGE_SIZE >= (sizeof(majorVersion) + sizeof(minorVersion) + sizeof(buildNumber))
                        ? EEP_PAGE_SIZE - (sizeof(majorVersion) + sizeof(minorVersion) + sizeof(buildNumber))
                        : 0]{};  // Expansion area
    };
    static_assert(sizeof(PageProductInfo3) <= EEP_PAGE_SIZE, "EeProm0PageProductInfo3 declaration too big!!");
    ...
    constexpr auto GRID1_CONTROL_DATA_PAGE_1 = PageId::PAGE_030;

    //----------PAGE_031---------------
    // Grid1 Control Data Page Two of Two
    constexpr auto GRID1_CONTROL_DATA_PAGE_2 = PageId::PAGE_031;

    //----------PAGE_032---------------
    // Grid2 Control Data Page One of Two
    constexpr auto GRID2_CONTROL_DATA_PAGE_1 = PageId::PAGE_032;

    //----------PAGE_033---------------
    // Grid2 Control Data Page Two of Two
    constexpr auto GRID2_CONTROL_DATA_PAGE_2 = PageId::PAGE_033;

    //----------PAGE_034---------------
    // Grid3 Control Data Page One of Two
    constexpr auto GRID3_CONTROL_DATA_PAGE_1 = PageId::PAGE_034;

    //----------PAGE_035---------------
    // Grid3 Control Data Page Two of Two
    constexpr auto GRID3_CONTROL_DATA_PAGE_2 = PageId::PAGE_035;

    //----------PAGE_036---------------
    // Heater Control Data Page One of Two
    constexpr auto HEATER_CONTROL_DATA_PAGE_1 = PageId::PAGE_036;

    //----------PAGE_037---------------
    // Heater Control Data Page Two of Two
    constexpr auto HEATER_CONTROL_DATA_PAGE_2 = PageId::PAGE_037;

    //----------PAGE_038---------------
    // HighVoltage Control Data Page One of Two
    constexpr auto HV_CONTROL_DATA_PAGE_1 = PageId::PAGE_038;

    //----------PAGE_039---------------
    // HighVoltage Control Data Page Two of Two
    constexpr auto HV_CONTROL_DATA_PAGE_2 = PageId::PAGE_039;
    ...
    constexpr auto SIGNATURE_PAGE = PageId::PAGE_127;
    ...
    struct PageSignature { // 1 byte 00, 5 'random' bytes, 1 byte 00, 4 bytes packed date
        ...
        uint8_t unused [EEP_PAGE_SIZE >= sizeof(signature)
                        ? EEP_PAGE_SIZE - sizeof(signature)
                        : 0]{};  // Expansion area
    };
    static_assert(sizeof(PageSignature) <= EEP_PAGE_SIZE, "EeProm0PageSignature declaration too big!!");
    constexpr uint8_t SIGNATURE_BUILD_DATE_OFFSET = 0x07; // location of the packed build date
    union PageSignatureBuffer {
        PageSignature data{};
        EepBuffer_t   buffer; // Initializer is necessary or struct is immediately destroyed.
    };
